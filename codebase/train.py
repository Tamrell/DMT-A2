...
####### convergence criterium? ######
            ...
            if i > 99 and i%100 == 0:
                print(f"{i}: {np.mean(trn_ndcg[-100:])}, loss: {np.mean(losses[-100:])}\n", end="\r")
...
##############################################
        ...
        print(f"Train NDCG: {np.mean(trn_ndcg):5}, Validation NDCG: {np.mean(val_ndcg):5}, t loss: {np.mean(losses):5}, model_id: {model_id}, (Epoch time: {time.time()-t})")
